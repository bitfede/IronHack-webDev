...
function checkObstacles(rover) {
 for (var i = 0; i < obstacles.length; i++) {
  if ((rover.position[1]) === (obstacles[i][1]) && (rover.position[0]) === (obstacles[i][0])) {
    return true; //the rover is potentially running against an obstacle
  }
}
for (var i = 0; i< rovers.length; i ++) { //loops thru the list of rovers in the map
  if (rover.name = rovers[i].name) {
    //do nothing because if not we will be comparing the positions for the same rover
  }
  else if ((rover.position[1]) === (rovers[i].position[1]) && (rover.position[0]) === (rovers[i].position[0])) {
...
function goForward(rover) {
  switch(rover.direction) {
    case 'N':
      rover.position[1]++
      var check = checkObstacles(rover);
